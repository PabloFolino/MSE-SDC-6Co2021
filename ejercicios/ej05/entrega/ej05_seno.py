...
if (L_FIR % 2 == 0):
    ...
    p_mod[1:] = sc.cosine(L_FIR-1)
else:
    p_mod = sc.cosine(L_FIR)
...
if (L_FIR % 2 == 0):
    ...
    p_dem[1:] = sc.cosine(L_FIR-1)
else:
    p_dem = sc.cosine(L_FIR)
...
p_axe.set_title("Pulso senoidal")
p_axe.stem(p_mod)
p_axe.set_xlabel("k")
...
real_axe_1.set_title("Pulso senoidal - Canal 1: Delta fase cero - Sigma_N: %f" % SIGMA_N)
...
real_axe_2.set_title("Pulso senoidal - Canal 2: Delta fase distina de cero - Sigma_N: %f" % SIGMA_N)
...
real_axe_3.set_title("Pulso senoidal - Canal 3: Pasabajos - Sigma_N: %f" % SIGMA_N)
